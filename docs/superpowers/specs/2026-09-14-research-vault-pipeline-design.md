...
週1（月曜朝を既定）。**Windows タスクスケジューラから 1 つのタスクで ③→①→② を順に実行する**
（2026-09-14 確定）。

②は `C:\Users\yuu18\.local\bin\claude.exe`（v2.1.268・ネイティブ exe・実在確認済み）を
`-p` でヘッドレス起動する。作業ディレクトリは本リポジトリ。

`mcp__scheduled-tasks__create_scheduled_task` は採らない。**デスクトップアプリが起動していない
時間帯に発火しない**ため、「毎週必ず回る」という要件を満たせない。週次で 1 回しか機会が
ないものを、アプリの起動状態に依存させるのは弱すぎる。
...
- `promote.py` が history へ書く際の、既存 router との実装共有の粒度
  （サービス関数を切り出すか、router を薄くして再利用するか）。実コードを読んで決める。

**解決済み:**

- ②の起動手段 → Windows タスクスケジューラ + `claude.exe -p`（2026-09-14、6.1 節）。
- `literature-vault-agent` リポへのコミット可否 → 可（2026-09-14、ユーザ承認）。
