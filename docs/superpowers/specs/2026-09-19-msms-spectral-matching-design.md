...
一方、**展開後のペイロードに対して**必要な情報は 1% 以下である（LZ4 圧縮済みの元ファイルとの比ではない）。容量の大半は `SpectrumPeak` が 1 ピークあたり
...
| store は永続化 | `.dbs` は全走査が不可避。**判断の根拠は容量ではなく時間**——実測で構築 12.7 秒 / 再オープン 0.084 秒（5.2） |
