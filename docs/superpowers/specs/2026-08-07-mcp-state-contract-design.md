...
`eic_plot_chromatograms` でも `eic_plot_compounds` でも作れる。

**連鎖が必要な依存（A を作るには先に B）は、クライアントが深さ上限つきで再帰的に解決する。**
当初この設計は「リトライ1段の範囲で自然に解決する」と書いていたが、これは誤りだった。
最終レビューで再現されたとおり、リプレイしたツール自身が `missing_state` を返した時点で
復旧が打ち切られる。MCP 再接続後は全状態が消えるため `preprocessed_matrix` /
`differential_result` / `pca_result` / `eic_plot` を要求するツールはすべて2段目以降になり、
実セッション 52ecf212 で実際に失敗した箇所がそのまま復旧不能になる。しかも失敗が
LLM への委譲に落ちると、LLM が前処理を**自分の選んだ引数で**再実行して解析条件が
黙って変わる — §5.3 が「コードで担保する」と定めた当のものが破れる。
...
4. 候補（OR。AND チェーンではない）のうち次を両方満たす呼び出しを全部集め、
   その中で **invocation id が最大＝直近に成功したもの**を1つ選ぶ
     ...
     - そのツール自体が現在のポリシーで実行を許されるか（承認判定を迂回しない）
5. 選べたら記録済み引数のまま再実行する。**それ自身が missing_state を返したら、
   深さ上限まで同じ手順を再帰的に適用する**（load → preprocess → pca のような連鎖を
   記録済み引数のまま復元するため）
6. 深さ上限は 3。ARF の実際の連鎖が3段（データ読込→前処理→PCA/差次）なので足り、
   循環や暴走は上限で必ず止まる
7. 復元できたら本命を1回だけリトライする
8. 1つも選べない／リプレイが失敗した／上限に達した場合は envelope の message を LLM へ渡す
     （Claude Desktop と同じ振る舞い。LLM が自分で前提ツールを呼べる）
9. 本命のリトライは1回のみ。リトライ結果はこの手順に再投入しない
```
...
- **リプレイ自体が失敗**: リプレイの失敗を開示し、本命はリトライしない。LLM に判断を委ねる。
  **リプレイが例外を送出した場合も同じ扱いにする**（タイムアウトやポリシー拒否）。捕捉しないと
  例外がターン全体を殺し、セッションが `status="error"` で止まる。95MB の ARF を
  `load_dataset` で再実行してタイムアウトする経路は現実的に起こりうる。
- **リプレイでは状態そのものしか戻らない**: `arf_exclude` のような「データセットに後から
  当てた修飾」は `required_tools` では表現できず、`arf_parser` / `load_dataset` の
  リプレイで消える。消えたことが黙って伝わらないと、外れ値を除外した解析が除外なしの
  統計に差し替わる。サーバは手動除外が**0件であることも積極的に開示**し、無言にしない。
