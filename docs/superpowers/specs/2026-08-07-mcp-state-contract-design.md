...
| `required_tools` | その状態を作るツール名（サーバ名を含まない裸の名前） | エラーを返したサーバの名前空間で解決する |
...
5. すべて満たせば、記録済み引数のまま順に再実行し、本命を1回だけリトライする
6. 満たせなければ envelope の message を tool メッセージとして LLM へ渡す
